...
def _rsi(rows, period):
    vals=[float(x["c"]) for x in rows]
    if len(vals)<period+2: return 50.0
    gains=[]; losses=[]
...
def _kdj_j(rows, n=9):
    if len(rows)<n: return 50.0
    window=rows[-n:]; hi=max(float(x["h"]) for x in window); lo=min(float(x["l"]) for x in window)
    rsv=50.0 if hi<=lo else (float(rows[-1]["c"])-lo)/(hi-lo)*100.0
    # lightweight current-bar J proxy: emphasize the stochastic extreme.
    return 3.0*rsv-2.0*50.0

def _shock(quarter, side):
    if len(quarter)<25: return False,{}
    q=indicators(quarter); prev=indicators(quarter[:-1])
    atr=float(q["atr"]); atr_prev=float(prev["atr"])
    width=(float(q["upper"])-float(q["lower"]))/abs(float(q["middle"]))
    pwidth=(float(prev["upper"])-float(prev["lower"]))/abs(float(prev["middle"]))
    atr_expand=atr_prev>0 and atr/atr_prev>=1.03
    boll_expand=pwidth>0 and width/pwidth>=1.03
    r6=_rsi(quarter,6); r24=_rsi(quarter,24); gap=(r24-r6) if side=="做多" else (r6-r24)
    j=_kdj_j(quarter)
    extreme=(j<=15.0) if side=="做多" else (j>=85.0)
    return (atr_expand or boll_expand) and gap>=15.0 and extreme,{"atr_expand":atr_expand,"boll_expand":boll_expand,"rsi6":r6,"rsi24":r24,"rsi_gap":gap,"kdj_j":j}
...
def _trigger(quarter,five,rs):
    ...
    shock_ok,shock=_shock(quarter,rs["side"])
    if not (zone and shock_ok and confirm): return None
    return {"bar_t":int(q["t"]),"signal_close_ms":int(q["t"])+15*60_000,
            "lower":lo,"middle":mean,"upper":hi,"reference":ref,"path":path,
            "bar_o":float(q["o"]),"bar_h":float(q["h"]),"bar_l":float(q["l"]),"bar_c":float(q["c"]),"shock":shock}

class TrendRangeMeanReversionModel:
    VERSION="V2.1-TrendMRShock-Research"
    BUILD=f"V210-{DAYS}D"
    ...
    def evaluate(hour,quarter,five,one,four,opportunity=None,stop_atr=1.0,
        ...
        if allow_new and trig:
            ...
            if opp is None or int(trig["bar_t"])>old:
                ...
                opp.update({"shock":dict(trig.get("shock") or {}),"regime_impulse_t":rs["t"],"impulse_volume_ratio":rs["volume_ratio"],
...
def main():
    ...
    print("V200_CONFIG",f"days={DAYS}","4H impulse: volume>=1.5x OR BOLL width>=1.2x",
          "range=post-impulse 4H","entry=trend-side half + ATR/BOLL shock + RSI24-RSI6 gap + KDJ extreme + 5m EMA20 reversal",
          "SL=1H ATR","TP=2R","no score/RSI/MACD/KDJ",flush=True)
    src.main()
    p=OUT/RESULT_NAME
    d=json.loads(p.read_text(encoding="utf-8"))
    d["research"]="V2.1 4H impulse -> range -> shock/extreme deviation -> 5m mean-reversion confirmation"
    d["strategy_lock"]={"impulse":"4H volume ratio >=1.50 OR BOLL width ratio >=1.20; candle/mid direction",
      ...
      "long":"bull regime + lower half + ATR/BOLL shock + RSI24-RSI6>=15 + KDJ J<=15 + bullish 5m EMA20 reclaim",
      "short":"bear regime + upper half + ATR/BOLL shock + RSI6-RSI24>=15 + KDJ J>=85 + bearish 5m EMA20 reject",
