#!/usr/bin/env python3
"""KAYTRADE V1.6.8 R2 + 30m BOLL lifecycle + ADX Hard Gate only, 180D.

Baseline V1.6.8 R2 is preserved:
- 15m latest CLOSED BOLL outer only, BOLL score +2;
...
Research deltas only:
1) 15m BOLL outer trigger remains valid for 30 minutes.
2) 5m ADX(14) Hard Gate ONLY: ADX>=25, opposite DI dominant, and ADX
   rises across 3 consecutive closed 5m bars => block entry.
3) ADX contributes 0 score; 1H EMA9/26 +1 is preserved.
"""
...
ADX_PERIOD = 14
ADX_THRESHOLD = 25.0
VERSION = "1.6.8-30M-ADX-HG-research"
BUILD = "1680-30m-adx-hg"
OUT = Path("backtest_output_v168_30m_adx_hardgate_180d")
...
def _install_30m_window():
    # Reuse the already-audited 30m lifecycle plumbing, but NOT its scoring model.
    adxsrc._install_window_patch()
...
def _decorate_adx_hard_gate(result, five):
    ...
    for side in ("做多", "做空"):
        ...
        # Preserve V1.6.8 scoring exactly. ADX only removes eligibility.
...
class HistoricalV168ThirtyMinAdxHardGateModel:
    VERSION = VERSION
    BUILD = BUILD
    ...
    _signal_window = staticmethod(adxsrc._signal_window_30m)
    ...
    def execution_checks(plan, opportunity, score):
        _install_30m_window()
        _ok, diag, blockers = base.HistoricalV168Model.execution_checks(plan, opportunity, score)
        diag = dict(diag or {})
        blockers = list(blockers or [])

        # Keep the proven R2 missing-now_ms fix, but normalize lifecycle wording
        # to the actual research rule: 30 minutes from the closed 15m trigger.
        normalized = []
        for blocker in blockers:
            text = str(blocker)
            if "15m BOLL信号已到下一根15m收盘" in text or ("5m外轨信号" in text and "下一根5m" in text):
                text = "V1.6.8 30m：15m BOLL触发已超过30分钟有效期"
            normalized.append(text)
        blockers = normalized

        conf = (score or {}).get("confirmations") or {}
        if bool(conf.get("5m_adx_hard_gate_block")):
            blockers.append("V1.6.8 30m ADX-HG：5m ADX强逆向 Hard Gate")
...
def _submit(self, result, now_ms, mark):
    ...
    base.v165runner.production = HistoricalV168ThirtyMinAdxHardGateModel
    try:
        before = self.pending
...
def configure():
    ...
    # base.configure() installs the baseline model; restore this research wrapper.
    _install_30m_window()
    base.research.VERSION = VERSION
    base.research.BUILD = BUILD
    base.research.model = HistoricalV168ThirtyMinAdxHardGateModel
...
def verify_lock():
    ...
    assert base.BOLL_WINDOW_MS == 1_800_000
    assert HistoricalV168ThirtyMinAdxHardGateModel.ENTRY_WINDOW_MS == 1_800_000
    assert HistoricalV168ThirtyMinAdxHardGateModel.THRESHOLD == 6.0
    assert HistoricalV168ThirtyMinAdxHardGateModel.BOLL_OUTER_SCORE == 2.0
    assert HistoricalV168ThirtyMinAdxHardGateModel.FIVE_MINUTE_BOLL_ENABLED is False
...
def main():
    ...
    print(
        "V168_30M_ADX_HARDGATE_180D_CONFIG",
    ...
    payload = {
        "research": "KAYTRADE V1.6.8 R2 + 30m BOLL + ADX Hard Gate only 180D",
        ...
        "correction": {
            ...
            "strategy_deltas": [
                "15m BOLL trigger lifecycle 15m -> 30m",
                "5m ADX14 strong-opposite Hard Gate only",
            ],
        },
    ...
    (OUT / "result_v168_30m_adx_hardgate_180d.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    base._write_csv(rows, OUT / "trades_v168_30m_adx_hardgate_180d.csv")
    (OUT / "README.txt").write_text(
        "KAYTRADE V1.6.8 R2 + 30m BOLL lifecycle + ADX Hard Gate only, 180D.\n"
        "Deltas: BOLL lifecycle 30m; ADX14>=25 + opposite DI + rising3 blocks entry.\n"
        "ADX score OFF; 1H EMA9/26 +1 preserved; PEE OFF.\n",
        encoding="utf-8",
    )

    print("V168_30M_ADX_HARDGATE_180D_RESULT", json.dumps(payload, ensure_ascii=False, indent=2), flush=True)
    print(
        f"V168_30M_ADX_HARDGATE_180D_TIMING total={elapsed:.2f}s "
