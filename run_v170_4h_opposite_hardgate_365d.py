#!/usr/bin/env python3
"""KAYTRADE V1.7.0 Build1700 365D backtest.
...
- A/B delta: 4H must be explicitly OPPOSITE to the trade direction; this opposite state becomes the mandatory Hard Gate and still scores +1;
...
src = pee4.src
DAYS = 365
OUT = Path("backtest_output_v170_4h_opposite_hardgate_365d")
LOCK_MS = 60 * 60_000
VERSION = "1.7.0"
BUILD = "1700"
...
_LOCK_UNTIL_MS = 0
_LOCK_EVENTS = []
_ORIG_FOUR_STATE = src.base.proven.prod.four_hour_state


def _four_state_opposite_required(four, side):
    """A/B: actual 4H trend must be opposite to the trade side.
    Return 'aligned' only when the opposite side is the actual aligned 4H state,
    so the inherited V1.7 hard-gate/scoring pipeline remains otherwise unchanged.
    """
    opposite = "做空" if side == "做多" else "做多" if side == "做空" else ""
    if not opposite:
        return "neutral"
    opposite_state = str(_ORIG_FOUR_STATE(four, opposite) or "neutral")
    own_state = str(_ORIG_FOUR_STATE(four, side) or "neutral")
    if opposite_state == "aligned":
        return "aligned"
    if own_state == "aligned":
        return "opposite"
    return "neutral"


def _patch_4h_opposite_gate():
    """Patch every historical facade that resolves the 4H trend state."""
    prod = src.base.proven.prod
    prod.four_hour_state = _four_state_opposite_required
    try:
        prod.base.four_hour_state = _four_state_opposite_required
    except Exception:
        pass
    try:
        prod.base.signal_core.four_hour_state = _four_state_opposite_required
    except Exception:
        pass
...
def _patch():
    ...
    _patch_4h_opposite_gate()

    pee4._patch()
    # PEE4 patch sets the verified tiered Boolean exit path. Keep V1.7 rescore and opposite-4H gate afterward.
    src.entry._rescore = _rescore_v170
    _patch_4h_opposite_gate()
...
def verify(start, end):
    ...
    assert src.base.proven.prod.four_hour_state is _four_state_opposite_required
    assert src.base.proven.prod.base.signal_core.four_hour_state is _four_state_opposite_required
...
def main():
    ...
    payload["research"] = "KAYTRADE V1.7.0 Build1700 A/B: 4H OPPOSITE mandatory Hard Gate +1, 365D, 2000U"
    ...
    s.update({
        ...
        "4h_opposite_hard_gate": True,
        "4h_opposite_hard_gate_score": 1.0,
    ...
    payload.setdefault("correction", {})["only_deltas"] = [
        ...
        "365D exact historical window",
        "ONLY A/B DELTA: replace 4H aligned Hard Gate +1 with 4H opposite Hard Gate +1",
        "remove 1H EMA9/26 +1",
    ...
    new = OUT / "result_v170_4h_opposite_hardgate_365d.json"
    ...
    for oldcsv in (
        ...
        if oldcsv.exists():
            oldcsv.rename(OUT / "trades_v170_4h_opposite_hardgate_365d.csv")
            break

    readme = OUT / "README.txt"
    readme.write_text(
        "KAYTRADE V1.7.0 Build1700 365D A/B backtest.\n"
        "ONLY DELTA: 4H opposite trend is mandatory Hard Gate +1 instead of 4H aligned. "
    ...
    print("V170_4H_OPPOSITE_HARDGATE_365D_RESULT", json.dumps(payload, ensure_ascii=False, indent=2), flush=True)
