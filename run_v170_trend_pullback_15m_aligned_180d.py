#!/usr/bin/env python3
"""KAYTRADE research: 4H+1H+15m aligned trend / direct 15m BOLL pullback, 180D.
...
OUT = Path("backtest_output_v170_trend_pullback_15m_aligned_180d")
RESULT_NAME = "result_v170_trend_pullback_15m_aligned_180d.json"
TRADES_NAME = "trades_v170_trend_pullback_15m_aligned_180d.csv"
TREND_TIMEFRAMES = ("4H", "1H", "15m")
...
def _joint_direction(hour, quarter, four):
    ...
    direction = h1 if h1 == m15 == h4 and h1 in ("做多", "做空") else "观望"
    ...
    def evaluate(hour, quarter, five, one, four, opportunity=None, stop_atr=1.0,
        ...
        if opp:
            if int(now_ms) >= int(opp.get("expires_ms") or 0):
                ...
                transition = ("invalidated", "4H/1H/15m趋势不再同向")
                opp = None
        ...
        if isinstance(opp, dict):
            ...
            scores[side] = {
                ...
                "confirmations": {
                    ...
                    "required": {
                        "4h_1h_15m_same_full_trend": direction == side,
...
def _submit_trend(self, result, now_ms, mark):
    ...
    factors = {
        ...
        "entry_rule": "4H+1H+15m same full trend + closed15m outer touch + Front>1.50R + Cost<0.30R",
        "position_multiplier": 1.0,
    }
...
def main():
    ...
    print(
        ...
        "entry=4H+1H+15m_same_full_trend_then_closed15m_BOLL_outer_direct",
    ...
    print("AUDIT PASS: closed bars + 4H/1H/15m same full trend + strict 15m outer touch + strict Front/Cost + exact PEE4", flush=True)
    ...
    payload = {
        "research": "4H+1H+15m aligned full-trend direct 15m BOLL outer pullback, strict Front>1.50R and Cost<0.30R, 180D",
        ...
        "strategy_lock": {
            ...
            "trend_requirement": "4H, 1H and 15m must all be the same full trend",
