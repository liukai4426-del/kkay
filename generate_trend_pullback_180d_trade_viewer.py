...
import run_trend_pullback_direct_180d as runner
from research_backtest_data_cache import load_market

ROOT = Path(__file__).resolve().parent
SOURCE_DIR = Path(os.environ.get("TRADE_VIEWER_SOURCE_DIR", str(runner.OUT)))
OUT_DIR = Path(os.environ.get("TRADE_VIEWER_OUT", str(runner.OUT)))
OUT_HTML = OUT_DIR / "KAYTRADE-Trend-Pullback-180D-Trade-Viewer.html"

TRADE_CSV_NAME = "trades_trend_pullback_direct_180d.csv"
RESULT_JSON_NAME = "result_trend_pullback_direct_180d.json"
...
def _load_candles():
    start, end, data, manifest = runner.load_viewer_market()
    candles = []
    for row in data["5m"]:
...
<div id="app">
  <aside class="sidebar">
    <div class="brand">
      ...
      <div class="sub">Trend Pullback 180D · 4H+1H趋势 / 15m外轨<br>点击订单后自动定位开仓/平仓位置。</div>
    </div>
    <div class="filters">
