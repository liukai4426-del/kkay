#!/usr/bin/env python3
"""KAYTRADE V1.7.0 Build1700 720D backtest.
...
Historical simulator only. Production files are unchanged.
"""
...
src = pee4.src
DAYS = 720
OUT = Path("backtest_output_v170_720d")
LOCK_MS = 60 * 60_000
VERSION = "1.7.0"
BUILD = "1700"
...
def _patch():
    ...
    pee4._patch()
    # PEE4 patch sets the verified tiered Boolean exit path. Keep V1.7 rescore afterward.
    src.entry._rescore = _rescore_v170
...
def main():
    ...
    payload["research"] = "KAYTRADE V1.7.0 Build1700 exact strategy replay, 720D, 2000U"
    ...
    for oldcsv in (
        ...
        if oldcsv.exists():
            oldcsv.rename(OUT / "trades_v170_720d.csv")
            break
    ...
    readme.write_text(
        ...
        "1H/15m primary-direction Hard Gate, 4H aligned Hard Gate, "
        "PEE4 first-4H/MFE0.60, global Lock1H, fixed 1x, 1H ATR SL, 2R TP, No-BE.\n",
        encoding="utf-8",
    )
    print("V170_720D_RESULT", json.dumps(payload, ensure_ascii=False, indent=2), flush=True)
