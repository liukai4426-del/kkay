    ...
    with tempfile.TemporaryDirectory(prefix='kaytrade-ui-check-') as folder:
        ...
        try:
            with patch.object(app.App,'worker',lambda self:None), patch.object(app.messagebox,'showerror',side_effect=AssertionError):
                ...
                assert 'KAYTRADE' in root.title() and '1.6.3' in root.title()
                ...
                assert label is not None and 'V1.6.3' in str(label.cget('text'))
                assert 'V1.6.3' in ui.signal.get()
                ui.stop(); assert ui.engine is None
                ui.finished.set(); ui.thread.join(timeout=2); ui.lock.close()
        finally:
            root.destroy()
    Path(sys.argv[2]).write_text('PASS: KAYTRADE V1.6.3 packaged UI startup; version sync and rounded Run Log scrollbar active; no network or orders.\n')
