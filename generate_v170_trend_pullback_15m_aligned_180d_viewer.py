...
import run_v170_trend_pullback_15m_aligned_180d as runner
from research_backtest_data_cache import load_market

ROOT = Path(__file__).resolve().parent
SOURCE_DIR = Path(os.environ.get("TRADE_VIEWER_SOURCE_DIR", "backtest_output_v170_trend_pullback_15m_aligned_180d"))
OUT_DIR = Path(os.environ.get("TRADE_VIEWER_OUT", "backtest_trade_viewer_v170_trend_pullback_15m_aligned_180d"))
OUT_HTML = OUT_DIR / "KAYTRADE-V1.7-Trend-Pullback-15m-Aligned-180D-Trade-Viewer.html"

TRADE_CSV_NAME = "trades_v170_trend_pullback_15m_aligned_180d.csv"
RESULT_JSON_NAME = "result_v170_trend_pullback_15m_aligned_180d.json"
...
def _html(candles, trades, result, manifest, start, end):
    payload = {
        ...
        "trades": trades,
        "meta": {
            "title": "KAYTRADE · Trend Pullback 15m同向 180D 回测交易K线审查",
            "source_run": "current-workflow",
...
<title>KAYTRADE · Trend Pullback 15m同向 180D 回测交易K线审查</title>
<style>
:root {{
  color-scheme: dark;
...
@media(max-width:900px){{#app{{grid-template-columns:200px 1fr}} .grid{{grid-template-columns:repeat(2,1fr)}} .bottom{{grid-template-columns:1fr;height:280px}} .details{{border-right:0;border-bottom:1px solid var(--line)}}}}
</style>
</head>
<body>
<div id="app">
  <aside class="sidebar">
    <div class="brand">
      ...
      <div class="sub">Trend Pullback · 15m同向 · 180D<br>点击订单后自动定位开仓/平仓位置。</div>
    </div>
    <div class="filters">
...
function updateDetails(){{
  ...
  const items=[
    ['方向',t.side],['Front R',Number.isFinite(Number(t.front_r))?num(t.front_r,2)+'R':'∞/未知'],['Cost R',num(t.cost_r,3)+'R'],['净收益',signed(t.net_pnl,2)+' U'],['Realized R',signed(t.realized_r,2)+'R'],
    ...
    ['MFE',signed(t.mfe_r,2)+'R'],['MAE',signed(t.mae_r,2)+'R'],['15m趋势',t.trend_15m||'—'],['1H趋势',t.trend_1h||'—'],['4H趋势',t.trend_4h||'—'],['前方结构周期',t.front_structure_timeframe||'—'],['BOLL路径',t.boll_path||'—'],
  ];
