...
- A/B delta: 4H aligned is allowed +1; 4H neutral is allowed +0; only explicit 4H opposite is a Hard Gate block;
...
src = pee4.src
DAYS = 365
OUT = Path("backtest_output_v170_4h_opposite_block_365d")
LOCK_MS = 60 * 60_000
VERSION = "1.7.0"
BUILD = "1700"
...
def _apply_4h_actual(result):
    ...
    aligned +1, neutral +0, opposite +0 and blocked.
    """
    ...
    for side in ("做多", "做空"):
        ...
        layers["trend4h"] = 1.0 if actual == "aligned" else 0.0
        row["layers"] = layers
        ...
        conf["4H_aligned_score"] = 1.0 if actual == "aligned" else 0.0
...
def verify(start, end):
    ...
    # A/B semantic lock: aligned +1/pass, neutral +0/pass, opposite +0/block.
    ...
    assert chk["scores"]["做多"]["layers"]["trend4h"] == 0.0
...
def main():
    ...
    payload["research"] = "KAYTRADE V1.7.0 Build1700 A/B: 4H explicit opposite blocked; aligned+1 and neutral+0 allowed, 365D, 2000U"
    ...
    payload.setdefault("correction", {})["only_deltas"] = [
        ...
        "ONLY A/B DELTA: 4H aligned +1 allowed; 4H neutral +0 allowed; explicit 4H opposite Hard Gate blocked",
        "remove 1H EMA9/26 +1",
    ...
    new = OUT / "result_v170_4h_opposite_block_365d.json"
    ...
    for oldcsv in (
        ...
        if oldcsv.exists():
            oldcsv.rename(OUT / "trades_v170_4h_opposite_block_365d.csv")
            break

    readme = OUT / "README.txt"
    readme.write_text(
        "KAYTRADE V1.7.0 Build1700 365D A/B backtest.\n"
        "ONLY DELTA: explicit 4H opposite blocks; aligned is allowed +1 and neutral is allowed +0. "
    ...
    print("V170_4H_OPPOSITE_BLOCK_365D_RESULT", json.dumps(payload, ensure_ascii=False, indent=2), flush=True)
