#!/usr/bin/env python3
"""KAYTRADE V1.7.0 Build1700 360D research backtest.
...
- A/B delta: 4H +1 score is based only on EMA20/EMA50 direction (long EMA20>EMA50; short EMA20<EMA50); explicit 4H opposite remains a Hard Gate block while aligned/neutral are allowed;\n- A/B delta: SHORT entry_drift_atr5 > 0.20 is a Hard Gate block;
...
src = pee4.src
DAYS = 360
OUT = Path("backtest_output_v170_4h_ema_score_short_drift020_360d")
LOCK_MS = 60 * 60_000
VERSION = "1.7.0"
BUILD = "1700"
...
def _four_state_opposite_only_block(four, side):
    """Gate: block only explicit full-state opposite. Score source: 4H EMA20/EMA50 direction."""
    actual = str(_ORIG_BASE_FOUR_STATE(four, side) or "neutral")
    _LAST_4H_ACTUAL[side] = actual

    ema20 = ema50 = math.nan
    ema_ok = False
    try:
        q = indicators(four)
        ema20 = float(q["ema20"])
        ema50 = float(q["ema50"])
        ema_ok = (ema20 > ema50) if side == "做多" else (ema20 < ema50) if side == "做空" else False
    except Exception:
        ema_ok = False
    _LAST_4H_EMA[side] = {"ema20": ema20, "ema50": ema50, "aligned": bool(ema_ok)}

    # The inherited gate only understands "aligned passes". Map neutral to aligned
    # for gate transport; preserve the actual full-state classification separately.
    return "opposite" if actual == "opposite" else "aligned"

def _apply_4h_actual(result):
    """Keep opposite-only Hard Gate; replace old 4H state +1 with EMA20/EMA50 trend +1."""
    ...
    for side in ("做多", "做空"):
        ...
        ema = _LAST_4H_EMA.get(side) or {}
        ema20 = ema.get("ema20")
        ema50 = ema.get("ema50")
        ema_ok = bool(ema.get("aligned"))

        layers = row.get("layers") if isinstance(row.get("layers"), dict) else {}
        layers["trend4h"] = 1.0 if ema_ok else 0.0
        row["layers"] = layers

        conf = row.setdefault("confirmations", {})
        conf["4H_trend_state_actual"] = actual
        conf["4H_gate_rule"] = "explicit opposite blocks; aligned and neutral allowed"
        conf["4H_neutral_allowed"] = True
        conf["4H_score_rule"] = "EMA20>EMA50 for long; EMA20<EMA50 for short"
        conf["4H_ema20"] = ema20
        conf["4H_ema50"] = ema50
        conf["4H_ema_trend_ok"] = ema_ok
        conf["4H_direction_score"] = 1.0 if ema_ok else 0.0
    ...
    result["4h_score_mode"] = "ema20_ema50_direction"
    return result
...
def _submit_lock(self, result, now_ms, mark):
    ...
    if self.pending is not None and self.pending is not before:
        ...
        self.pending.factors["4h_gate_mode"] = "opposite_only_block"
        self.pending.factors["4h_score_mode"] = "ema20_ema50_direction"
        self.pending.factors["4h_ema20"] = conf.get("4H_ema20")
        self.pending.factors["4h_ema50"] = conf.get("4H_ema50")
        self.pending.factors["4h_ema_trend_ok"] = bool(conf.get("4H_ema_trend_ok"))
...
def verify(start, end):
    ...
    # A/B semantic lock: opposite blocks; score comes only from EMA20/EMA50 direction.
    global _LAST_4H_ACTUAL, _LAST_4H_EMA
    sample4 = {
        "scores": {
    ...
    _LAST_4H_EMA = {
        "做多": {"ema20": 101.0, "ema50": 100.0, "aligned": True},
        "做空": {"ema20": 101.0, "ema50": 100.0, "aligned": False},
    }
    chk = _apply_4h_actual(sample4)
    assert chk["scores"]["做多"]["layers"]["trend4h"] == 1.0
    assert chk["scores"]["做多"]["confirmations"]["4H_ema_trend_ok"] is True
...
def main():
    ...
    payload["research"] = "KAYTRADE V1.7.0 Build1700 A/B: 4H explicit opposite blocked; aligned+1 and EMA-score allowed; SHORT Entry Drift >0.20 ATR5 blocked, 360D, 2000U"
    payload["window"]["days"] = DAYS
    payload.setdefault("metrics", {})["days"] = DAYS
    ...
    s.update({
        ...
        "4h_aligned_score": None,
        "4h_neutral_allowed": True,
        "4h_neutral_score": None,
        "4h_score_source": "EMA20/EMA50 direction",
        "4h_ema_trend_score": 1.0,
        "4h_ema_long_rule": "EMA20>EMA50",
        "4h_ema_short_rule": "EMA20<EMA50",
    ...
    payload.setdefault("correction", {})["only_deltas"] = [
        ...
        "A/B DELTA: explicit 4H opposite Hard Gate blocked; aligned/neutral allowed",
        "A/B DELTA: 4H +1 score uses EMA20/EMA50 direction only",
    ...
    new = OUT / "result_v170_4h_ema_score_short_drift020_360d.json"
    ...
    for oldcsv in (
        ...
        if oldcsv.exists():
            oldcsv.rename(OUT / "trades_v170_4h_ema_score_short_drift020_360d.csv")
            break

    readme = OUT / "README.txt"
    readme.write_text(
        "KAYTRADE V1.7.0 Build1700 360D A/B backtest.\n"
        "A/B DELTAS: explicit 4H opposite blocks; aligned/neutral allowed; 4H +1 score uses EMA20/EMA50 direction only; SHORT Entry Drift >0.20 ATR5 blocks. "
    ...
    print("V170_4H_EMA_SCORE_SHORT_DRIFT020_360D_RESULT", json.dumps(payload, ensure_ascii=False, indent=2), flush=True)
