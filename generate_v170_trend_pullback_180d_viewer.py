...
import run_v170_trend_pullback_180d as runner
from research_backtest_data_cache import load_market

ROOT = Path(__file__).resolve().parent
SOURCE_DIR = Path(os.environ.get("TRADE_VIEWER_SOURCE_DIR", "backtest_output_v170_trend_pullback_180d"))
OUT_DIR = Path(os.environ.get("TRADE_VIEWER_OUT", "backtest_trade_viewer_v170_trend_pullback_180d"))
OUT_HTML = OUT_DIR / "KAYTRADE-V1.7-Trend-Pullback-180D-Trade-Viewer.html"

TRADE_CSV_NAME = "trades_v170_trend_pullback_180d.csv"
RESULT_JSON_NAME = "result_v170_trend_pullback_180d.json"
...
def _html(candles, trades, result, manifest, start, end):
    payload = {
        ...
        "trades": trades,
        "meta": {
            "title": "KAYTRADE · Trend Pullback 180D 回测交易K线审查",
...
<div id="app">
  <aside class="sidebar">
    <div class="brand">
      ...
      <div class="sub">Trend Pullback · 180D<br>点击订单后自动定位开仓/平仓位置。</div>
    </div>
    <div class="filters">
...
function updateDetails(){{
  ...
  const items=[
    ...
    ['MFE',signed(t.mfe_r,2)+'R'],['MAE',signed(t.mae_r,2)+'R'],['1H趋势',t.trend_1h||'—'],['4H趋势',t.trend_4h||'—'],['前方结构周期',t.front_structure_timeframe||'—'],['BOLL路径',t.boll_path||'—'],
  ];
