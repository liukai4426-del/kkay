...
def okx_get(path, params, retries=6):
    ...
    req=urllib.request.Request(url,headers={'Accept':'application/json','User-Agent':'kaytrade-v13-backtest/1.0'})
...
def fetch_5m():
    ...
    if gaps: raise RuntimeError(f'5m gaps: {gaps[:5]} total={len(gaps)}')
...
def exact_signals(five,q,h):
    ...
    for i5,r in enumerate(five):
        ...
        value=signal(hw,qw,fw,THRESHOLD,STOP_ATR)
        checked+=1
        if value['side']!='观望':
        ...
        if checked%3000==0: print(f'Exact V1.3 score evaluations: {checked}',flush=True)
    print(f'Exact eligible V1.3 bars: {len(out)}',flush=True)
    return out
...
def simulate(five,signals,fee_bps=FEE_BPS,slip_bps=EXIT_SLIP_BPS,start_equity=1000.0,max_mult=5.0,apply_streak=True):
    trades=[]; pos=None; pending=None; cooldown_until=0
    equity=start_equity; peak=equity; max_dd=0.0
    submitted=filled=canceled=cost_blocked=streak_blocked=0; streak_stop_days=set(); same_entry_bar_exits=0
    streak=0; streak_day=''
    ...
    for i,r in enumerate(five):
        ...
        if pending and t==pending['entry_bar_t']:
            ...
            if touched:
                ...
                if hit_sl or hit_tp:
                    ...
                    trades.append({**pos,'exit_t':t+STEP5,'exit':exit_fill,'trigger':trigger,'r':gross_r,'reason':reason,'net_pnl':net,'equity_after':equity})
        ...
        if pos:
            d=pos['d']; hit_sl=(r['l']<=pos['sl']) if d==1 else (r['h']>=pos['sl']); hit_tp=(r['h']>=pos['tp']) if d==1 else (r['l']<=pos['tp'])
            if hit_sl or hit_tp:
                reason='SL' if hit_sl else 'TP'; trigger=pos['sl'] if hit_sl else pos['tp']; gross_r=-1.0 if hit_sl else REWARD_R
                exit_fill=trigger*(1-d*slip)
    ...
    gross_wins=[t for t in trades if t['r']>0]; gross_losses=[t for t in trades if t['r']<0]
    ...
    return {
        ...
        'max_dd_pct':100*max_dd,'net_wins':len(wins),'net_losses':len(losses),'net_win_rate_pct':100*len(wins)/len(trades) if trades else 0,
        'net_profit_factor':gp/gl if gl else None,'gross_wins':len(gross_wins),'gross_losses':len(gross_losses),
        'gross_win_rate_pct':100*len(gross_wins)/len(trades) if trades else 0,
        ...
        'limit_fill_rate_pct':100*filled/submitted if submitted else 0,'cost_blocked':cost_blocked,'streak_blocked':streak_blocked,
...
def summarize_group(rows,key):
    ...
    for k,v in groups.items():
        ...
        out[k]={'trades':len(v),'wins':len(wins),'losses':len(v)-len(wins),'win_rate_pct':100*len(wins)/len(v),
...
def main():
    ...
    signals=exact_signals(five,q,h)
    base=simulate(five,signals)
    low_cost=simulate(five,signals,fee_bps=5,slip_bps=2)
    zero_cost=simulate(five,signals,fee_bps=0,slip_bps=0)
    trades=base['trades']
    ...
    report={
        ...
        'source':'OKX public BTC-USDT-SWAP 5m history-candles; 15m/1H resampled from 5m',
        'strategy_version':'KAYTRADE V1.3 exact strategy.signal scoring, rolling up to 1500 closed bars per timeframe',
        'market':{'five_min_candles':len(five),'fifteen_min_candles':len(q),'hour_candles':len(h),'start_price':market_start,'end_price':market_end,'btc_return_pct':100*(market_end/market_start-1)},
        'rules':{'score_max':18,'threshold':8,'countertrend_threshold':11,'setup_min':2,'trigger_min':1,'stop':'1.0x 15m ATR','target':'1.5R','cooldown_min':30,'forward_structure':'<1R block; 1.0-1.3R -2 points','cost_filter':'TP distance >= 2x budgeted round-trip execution cost','limit_timeout':'one 5m candle','china_day_loss_streak_stop':3},
        'execution_assumptions':{'entry_limit_proxy':'signal 5m close, filled only if next 5m candle trades through the limit; otherwise canceled','historical_bid_ask':'not available, spread/chase filter not reconstructed','partial_fills':'not reconstructable from OHLC; modeled full fill or no fill','same_entry_bar':'SL takes priority if both SL/TP appear in same 5m candle','funding':'excluded','daily_equity_loss_cap':'excluded because user-local absolute USDT setting is unknown','net_base':'1000 USDT normalized equity; 1% equity risk target; max 5x notional; 10bps fee each side; 5bps adverse market-exit slippage; limit entry has no slippage','three_loss_guard':'applied on net PnL, resets at Asia/Shanghai natural-day boundary'},
        'eligible_signal_bars':len(signals),
        'base':{k:v for k,v in base.items() if k!='trades'},
        ...
        'by_level':summarize_group(trades,'level'),
        'by_score':summarize_group(trades,'score'),
        'by_month':{},
    }
    ...
    report['by_month']={k:{'trades':len(v),'wins':sum(x['net_pnl']>0 for x in v),'net_pnl':sum(x['net_pnl'] for x in v),'gross_r':sum(x['r'] for x in v)} for k,v in sorted(by_month.items())}
    with open('v13_backtest_report.json','w') as f: json.dump(report,f,ensure_ascii=False,indent=2)
    fields=['signal_t','entry_t','exit_t','side','score','level','signal_close','entry','sl','tp','trigger','exit','r','reason','net_pnl','equity_before','equity_after','cost_multiple','front_r','strong_opposite']
    with open('v13_backtest_trades.csv','w',newline='') as f:
