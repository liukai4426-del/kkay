...
# V1.6.0 preserves the single-runtime-hook packaging rule introduced by Build1551.
# Only the final V1.6.0 overlay runs as a PyInstaller runtime hook; it imports
# and applies the inherited V1.5.6 stack exactly once as normal modules.
a = Analysis([str(root / 'desktop.py')], pathex=[str(root)],
             hiddenimports=['unittest.mock', 'v152_model', 'v153_model', 'v154_model', 'v154_limit_log_fix', 'v154_record_card_boll_fix', 'v154_fixed_card_precision_fix', 'v154_build1543_patch', 'v154_build1543_ui_cleanup', 'v154_build1544_patch', 'v155_update_patch', 'v155_build1551_patch', 'v156_execution_hotfix', 'v156_ui_patch', 'v156_build1561_patch', 'v160_model', 'v160_update_patch'], datas=[(str(root / 'assets' / 'kaytrade-v142-logo.png'),'assets')], binaries=[],
             hookspath=[], runtime_hooks=[str(root / 'v160_update_patch.py')], excludes=[])
pyz = PYZ(a.pure)
...
bundle = BUNDLE(collection, name='KAYTRADE.app',icon=str(root / 'OKXLocal.icns'),
                ...
                info_plist={'CFBundleShortVersionString': '1.6.0',
                            'CFBundleVersion': '1600',
