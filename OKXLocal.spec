...
# Only the final Build1561 overlay runs as a PyInstaller runtime hook; it imports
# and applies the inherited V1.5.6 patch chain exactly once as normal modules.
a = Analysis([str(root / 'desktop.py')], pathex=[str(root)],
             ...
             hookspath=[], runtime_hooks=[str(root / 'v156_build1561_patch.py')], excludes=[])
pyz = PYZ(a.pure)
