#!/usr/bin/env python3
"""KAYTRADE V1.7.0 Build1700 720D structure-distance instrumentation backtest.
...
src = pee4.src
DAYS = 720
OUT = Path("backtest_output_v170_structure_distance_record_720d")
LOCK_MS = 60 * 60_000
VERSION = "1.7.0"
BUILD = "1700"
...
def _structure_distance_diag(opportunity):
    """Record-only structure distance using the same zone-distance definition as Structure Overlap."""
    opp = opportunity if isinstance(opportunity, dict) else {}
    structure = opp.get("structure") if isinstance(opp.get("structure"), dict) else None
    try:
        atr15 = float(opp.get("atr15"))
        zone_low = float(opp.get("aux_zone_low"))
        zone_high = float(opp.get("aux_zone_high"))
        price = float(structure.get("price")) if structure else math.nan
    except (TypeError, ValueError):
        atr15 = 0.0
        zone_low = zone_high = price = math.nan
    if not (math.isfinite(atr15) and atr15 > 0 and math.isfinite(zone_low) and math.isfinite(zone_high) and math.isfinite(price)):
        return {"structure_distance_atr15": None, "structure_price": None, "structure_zone_low": None, "structure_zone_high": None}
    distance = 0.0 if zone_low <= price <= zone_high else min(abs(price - zone_low), abs(price - zone_high))
    return {
        "structure_distance_atr15": distance / atr15,
        "structure_price": price,
        "structure_zone_low": zone_low,
        "structure_zone_high": zone_high,
    }
...
def main():
    ...
    payload["research"] = "KAYTRADE V1.7.0 Build1700 720D RECORD-ONLY STRUCTURE DISTANCE: baseline unchanged; logs structure_distance_atr15, 2000U"
    ...
    new = OUT / "result_v170_structure_distance_record_720d.json"
    ...
    for oldcsv in (
        ...
        if oldcsv.exists():
            oldcsv.rename(OUT / "trades_v170_structure_distance_record_720d.csv")
            break

    readme = OUT / "README.txt"
    readme.write_text(
        "KAYTRADE V1.7.0 Build1700 720D structure-distance record-only backtest.\n"
