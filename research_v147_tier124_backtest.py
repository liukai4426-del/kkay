#!/usr/bin/env python3
"""V1.4.7 BTC 30-day comparison: minimum score 4.0 with tier caps in 1:2:4 ratio.
...
Tier 1 / first signal = 500 USDT (1x)
Tier 2 / second signal = 1,000 USDT (2x)
Tier 3 / third signal = 2,000 USDT (4x)
"""
...
MIN_ENTRY_SCORE = 4.0
TIER_CAP = {1: 500.0, 2: 1000.0, 3: 2000.0}
END = datetime(2026, 9, 12, 7, 40, tzinfo=timezone.utc)
START = END - timedelta(days=30)
START_MS = int(START.timestamp() * 1000)
END_MS = int(END.timestamp() * 1000)
OUTDIR = Path('backtest_output_v147_tier124')
...
def write_report(sim, metrics, leg_net, cycles, closed):
    ...
    renames = {
        'V147_BTC_SWAP_30D_2000U_Trades.csv': 'V147_BTC_SWAP_30D_2000U_TIER124_Trades.csv',
        'V147_BTC_SWAP_30D_2000U_Cycles.csv': 'V147_BTC_SWAP_30D_2000U_TIER124_Cycles.csv',
        'V147_BTC_SWAP_30D_2000U_DailyEquity.csv': 'V147_BTC_SWAP_30D_2000U_TIER124_DailyEquity.csv',
        'V147_BTC_SWAP_30D_2000U_Backtest_Report.md': 'V147_BTC_SWAP_30D_2000U_TIER124_Backtest_Report.md',
    }
    ...
    report = OUTDIR / 'V147_BTC_SWAP_30D_2000U_TIER124_Backtest_Report.md'
    text = report.read_text(encoding='utf-8')
    text = text.replace(
        '# KAYTRADE V1.4.7 — BTC-USDT-SWAP 最近30天回测报告',
        '# KAYTRADE V1.4.7 — BTC-USDT-SWAP 最近30天 1:2:4仓位比例 回测报告'
    )
    text = text.replace(
        'Tier名义仓位上限1,000/1,500/2,000U。',
        '第一/第二/第三信号名义仓位上限500/1,000/2,000U，对应1x/2x/4x。'
    )
    text += (
        '\n\n**本情景唯一策略变化：** 最低开仓门槛保持4.0分；第一/第二/第三信号仓位按'
        '500/1,000/2,000 USDT配置（1:2:4）。SL=15m ATR×1.0、TP=2R及其他规则全部不变。\n'
    )
