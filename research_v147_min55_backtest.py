#!/usr/bin/env python3
"""V1.4.7 BTC 30-day comparison: minimum entry score 5.5 only.
...
OUTDIR = Path('backtest_output_v147_min55')
...
v.V147Market.score = score_min55
...
def write_report(sim, metrics, leg_net, cycles, closed):
    ...
    renames = {
        'V147_BTC_SWAP_30D_2000U_Trades.csv': 'V147_BTC_SWAP_30D_2000U_MIN55_Trades.csv',
        'V147_BTC_SWAP_30D_2000U_Cycles.csv': 'V147_BTC_SWAP_30D_2000U_MIN55_Cycles.csv',
        'V147_BTC_SWAP_30D_2000U_DailyEquity.csv': 'V147_BTC_SWAP_30D_2000U_MIN55_DailyEquity.csv',
        'V147_BTC_SWAP_30D_2000U_Backtest_Report.md': 'V147_BTC_SWAP_30D_2000U_MIN55_Backtest_Report.md',
    }
    ...
    report = OUTDIR / 'V147_BTC_SWAP_30D_2000U_MIN55_Backtest_Report.md'
    text = report.read_text(encoding='utf-8')
    text = text.replace(
        '# KAYTRADE V1.4.7 — BTC-USDT-SWAP 最近30天回测报告',
        '# KAYTRADE V1.4.7 — BTC-USDT-SWAP 最近30天 最低5.5分 回测报告'
    )
    text = text.replace(
        '- 最低4分；Tier1 4.0–6.0、Tier2 6.5–7.5、Tier3 8.0–10.0；多空同时合格只取高分侧，同分观望。',
        '- 最低开仓门槛提高为5.5分；Tier映射仍保持原V1.4.7定义，4.0/4.5/5.0信号不允许开仓；其他多空仲裁不变。'
    )
    text += '\n\n**本情景唯一策略变化：** 自动开仓最低分从4.0提高到5.5；SL=15m ATR×1.0、TP=2R及其他规则全部不变。\n'
