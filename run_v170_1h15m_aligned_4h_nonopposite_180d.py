...
Research only. Production V1.7 files are not modified.\nWorkflow trigger: 1H+15m aligned / 4H non-opposite 180D A/B.
"""
...
OUT = Path("backtest_output_v170_1h15m_aligned_4h_nonopposite_180d")
RESULT_NAME = "result_v170_1h15m_aligned_4h_nonopposite_180d.json"
TRADES_NAME = "trades_v170_1h15m_aligned_4h_nonopposite_180d.csv"
...
def _joint_direction(hour, quarter, four):
    h1 = _trend_state(hour)
    m15 = _trend_state(quarter)
    h4 = _trend_state(four)
    direction = "观望"
    if h1 == m15 and h1 in ("做多", "做空"):
        opposite = "做空" if h1 == "做多" else "做多"
        if h4 != opposite:
            direction = h1
    return direction, h1, m15, h4
...
def _make_opportunity(hour, quarter, five, one, four, side, trigger):
    ...
    return {
        ...
        "trend_15m": _trend_state(quarter),
...
class TrendPullbackModel:
    ...
    def evaluate(hour, quarter, five, one, four, opportunity=None, stop_atr=1.0,
        ...
        direction, h1_state, m15_state, h4_state = _joint_direction(hour, quarter, four)
        ...
        if opp:
            if int(now_ms) >= int(opp.get("expires_ms") or 0):
                ...
                transition = ("invalidated", "1H/15m趋势不再同向或4H转为明确逆向")
                opp = None
        ...
        if isinstance(opp, dict):
            ...
            scores[side] = {
                ...
                "confirmations": {
                    ...
                    "required": {
                        "1h_15m_same_full_trend_4h_nonopposite": direction == side,
...
def _submit_trend(self, result, now_ms, mark):
    ...
    factors = {
        ...
        "trend_15m": str(opp.get("trend_15m") or ""),
        "trend_4h": str(opp.get("trend_4h") or ""),
        "4h_trend_state_actual": str(opp.get("trend_4h") or ""),
        ...
        "entry_rule": "1H+15m same full trend + 4H non-opposite + closed15m outer touch + Front>1.50R + Cost<0.30R",
        "position_multiplier": 1.0,
    }
...
def main():
    ...
    print(
        ...
        "entry=1H+15m_same_full_trend_4H_nonopposite_then_closed15m_BOLL_outer_direct",
    ...
    print("AUDIT PASS: closed bars + 1H/15m same full trend + 4H non-opposite + strict 15m outer touch + strict Front/Cost + exact PEE4", flush=True)
    ...
    payload = {
        "research": "1H+15m aligned full-trend with 4H non-opposite gate, direct 15m BOLL outer pullback, strict Front>1.50R and Cost<0.30R, 180D",
        ...
        "strategy_lock": {
            ...
            "15m_trend": "close vs EMA200 + EMA20/EMA50 ordering + both EMA slopes via indicators up/down",
            "trend_requirement": "1H and 15m must be the same full trend; 4H same direction or neutral passes, explicit opposite blocks",
