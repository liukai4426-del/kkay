...
src = pee4.src
DAYS = 720
OUT = Path("backtest_output_v170_short_drift_window020_720d")
LOCK_MS = 60 * 60_000
VERSION = "1.7.0"
BUILD = "1700"
...
def _submit_lock(self, result, now_ms, mark):
    ...
    if side == "做空":
        try:
            ...
            drift = math.nan
        if math.isfinite(drift) and (drift < SHORT_ENTRY_DRIFT_MIN_ATR5 or drift > SHORT_ENTRY_DRIFT_MAX_ATR5):
            self.stats["short_entry_drift_outside_window_block"] += 1
            self.stats["execution_gate_blocks"] += 1
            self.blockers["V1.7 A/B：做空 Entry Drift 不在[-0.20,+0.20] ATR5，Hard Gate"] += 1
            return
...
def main():
    ...
    payload["research"] = "KAYTRADE V1.7.0 Build1700 720D A/B: 4H explicit opposite blocked; aligned+1 and neutral+1; SHORT Entry Drift restricted to [-0.20,+0.20] ATR5, 2000U"
    ...
    payload.setdefault("correction", {})["only_deltas"] = [
        ...
        "A/B DELTA: SHORT Entry Drift outside [-0.20,+0.20] ATR5 Hard Gate blocked",
        "remove 1H EMA9/26 +1",
    ...
    new = OUT / "result_v170_short_drift_window020_720d.json"
    ...
    for oldcsv in (
        ...
        if oldcsv.exists():
            oldcsv.rename(OUT / "trades_v170_short_drift_window020_720d.csv")
            break

    readme = OUT / "README.txt"
    readme.write_text(
        "KAYTRADE V1.7.0 Build1700 720D Short Drift bounded-window A/B backtest.\n"
