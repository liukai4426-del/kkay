#!/usr/bin/env python3
"""V1.4.7 BTC 30-day comparison: TP/SL=1.6:1, stop=15m ATR x1.2.

Uses the same V1.4.7 signals, 2,000 USDT profile, fee model and exact 30-day
sample as the preceding comparison runs. Stop distance is 1.2 x 15m ATR and
target distance is 1.6R = 1.92 x 15m ATR.
"""
...
OUTDIR = Path('backtest_output_v147_1p6r_atr12')
...
def plan_1p6r(self, side, ti, score, signal_price, atr15, equity, daily_remaining):
    b = v.base
    buy = side == '做多'
...
def handle_open_leg_exit_1p6r(self, leg, bar, just_filled=False):
    ...
    if hit_sl:
        ...
        return
    if hit_tp:
        self.close_leg(leg, leg.tp, bar['t'], 'TP_1.6R')


v.base.Simulator.plan = plan_1p6r
v.base.Simulator.handle_open_leg_exit = handle_open_leg_exit_1p6r
...
def write_report(sim, metrics, leg_net, cycles, closed):
    ...
    renames = {
        'V147_BTC_SWAP_30D_2000U_Trades.csv': 'V147_BTC_SWAP_30D_2000U_1P6R_ATR12_Trades.csv',
        'V147_BTC_SWAP_30D_2000U_Cycles.csv': 'V147_BTC_SWAP_30D_2000U_1P6R_ATR12_Cycles.csv',
        'V147_BTC_SWAP_30D_2000U_DailyEquity.csv': 'V147_BTC_SWAP_30D_2000U_1P6R_ATR12_DailyEquity.csv',
        'V147_BTC_SWAP_30D_2000U_Backtest_Report.md': 'V147_BTC_SWAP_30D_2000U_1P6R_ATR12_Backtest_Report.md',
    }
    ...
    report = OUTDIR / 'V147_BTC_SWAP_30D_2000U_1P6R_ATR12_Backtest_Report.md'
    text = report.read_text(encoding='utf-8')
    text = text.replace(
        '**策略：** V1.4.7，1:2整仓止盈止损，15m ATR×1止损。',
        '**策略：** V1.4.7，盈亏比1.6:1；止损距离=15m ATR×1.2，止盈距离=15m ATR×1.92。'
    )
    text = text.replace(
        '# KAYTRADE V1.4.7 — BTC-USDT-SWAP 最近30天回测报告',
        '# KAYTRADE V1.4.7 — BTC-USDT-SWAP 最近30天 1.6:1 / ATR1.2 回测报告'
    )
